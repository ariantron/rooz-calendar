...
```
packages/calendar-core   @rooz-calendar/core — date engine + grid generation, no React
packages/calendar-ui     @rooz-calendar/ui   — shadcn-styled React components
apps/web                 the website — landing page, live demo and API reference in one app
```
...
The landing page, the interactive demo and the API reference are **one React application** in
`apps/web`, not three separate builds. It routes with React Router:

| Route   | What it is                                                                       |
| ------- | -------------------------------------------------------------------------------- |
| `/`     | What the library is, why it exists, and two *live* MonthViews — the same events rendered in Gregorian and in Jalali, side by side |
| `/demo` | The full playground: every view, both calendar systems, RTL, digit shaping, theme |
| `/docs` | The API reference, generated from the TypeScript sources at build time            |

```bash
pnpm dev             # serves the whole site on 0.0.0.0:5174
pnpm build:site      # static build into apps/web/dist
```

The demo imports `@rooz-calendar/ui` through its package entry point and the built `dist`,
exactly as an outside consumer does — so the demo cannot quietly diverge from what ships.

The API reference is generated by `apps/web/scripts/generate-api.mjs`, which reads both
packages' public entry points with the TypeScript compiler. Prop names, types, defaults and
descriptions all come from the code, so the page cannot drift away from the actual API.

`pnpm build:site` produces a static site that can be hosted anywhere. It uses real paths
rather than hash routes, so a static host needs two things: serve `index.html` for unknown
paths (the build emits a `404.html` copy, which is what GitHub Pages uses), and set
`VITE_BASE=/repo-name/` if the site is served from a subpath rather than a domain root.
...
pnpm typecheck       # every package and the site
pnpm build           # build both packages
pnpm dev             # the website at 0.0.0.0:5174 — /, /demo and /docs
